...
for k = 1:K % K iterations
    ...
    cv_classout = classify(all_samples(cv_test_idx,:),all_samples(cv_train_idx,:),all_labels(cv_train_idx));%classifies each row of the data in SAMPLE into one of the groups in TRAINING.
    ...
    cv_sensitivity(k) = TP/(TP+FN); % compute sensitivity
    cv_specificity(k) = TN/(TN+FP); % compute specificity
end
